...
import { getBusinessByCode } from "@/lib/business-utils";
...
export default function ReviewPage() {
  ...
  useEffect(() => {
    if (code) {
      const business = getBusinessByCode(code, mockBusinesses);
      if (business) {
        setBusinessId(business.id);
        // Store in sessionStorage for use in feedback flow
        sessionStorage.setItem("businessId", business.id);
        sessionStorage.setItem("businessCode", code);
      }
    }
  }, [code]);

  const handleRatingClick = (rating: "excellent" | "good" | "not-expected") => {
    if (rating === "excellent" || rating === "good") {
      router.push(`/feedback?rating=${rating}&code=${code}`);
    } else {
      router.push(`/manual-feedback?code=${code}`);
    }
  };
    ...
    <main className="h-screen sm:min-h-screen bg-gradient-to-br from-[#F7F1FF] via-[#F3EBFF] to-[#EFE5FF] flex flex-col items-center justify-center p-4 sm:p-6 overflow-hidden">
      <div className="w-full max-w-md space-y-4 sm:space-y-6">
        ...
        <div className="flex flex-col items-center space-y-6">
          ...
          <Button
            variant="secondary"
            onClick={() => handleRatingClick("not-expected")}
            className="w-3/4 h-16 sm:h-20 rounded-full text-base font-medium bg-white border border-[#9747FF] text-[#9747FF] hover:bg-white/90 shadow-[0px_4px_0px_#9747FF]"
          >
            <span className="text-xl mr-2">😕</span>
            Not Expected
